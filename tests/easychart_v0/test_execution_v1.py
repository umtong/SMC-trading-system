...
def test_market_entry_uses_taker_slippage_and_fee() -> None:
    ...
    assert adverse_stop_fill_price(
        side=Side.LONG, stop_price=90.0, costs=costs()
    ) == pytest.approx(89.982)
    assert adverse_stop_fill_price(
        side=Side.SHORT, stop_price=110.0, costs=costs()
...
def test_funding_validation_rejects_bad_mark_duplicate_or_missing_settlement() -> None:
    with pytest.raises(ValueError, match="mark_price"):
        FundingObservation("BTCUSDT", T0, 0.001, float("nan"))

    duplicate = FundingObservation(
        "BTCUSDT", T0 + pd.Timedelta(hours=2), 0.001, 100.0
    )
