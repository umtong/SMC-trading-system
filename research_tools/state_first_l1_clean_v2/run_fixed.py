...
def _strict_evaluate_command(args):
    ...
    The frozen implementation calibrated absolute-score quantiles from the full
    selection day.  Although that used no returns, it made early selection-day
    decisions depend on feature values observed later that day.  This wrapper,
    recorded before candidate PnL was observed, calibrates every threshold from
    the completed training day only and reuses the identical admitted signal set
    under 12/18/24 bp costs.
    """
    ...
    for (family, h), pred in predictions.items():
        # Strictly past calibration: only the completed training day is used.
