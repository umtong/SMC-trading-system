...
SYMBOLS_CONTEXT = ("SOLUSDT", "XRPUSDT")
...
def http_bytes(url: str) -> bytes:
    ...
    for attempt in range(7):
        try:
            request = urllib.request.Request(url, headers={"User-Agent": "v25p-frozen-public/1.0"})
            with urllib.request.urlopen(request, timeout=300) as response:
                return response.read()
        except Exception as exc:  # preserve final exception after bounded retries
            last = exc
            time.sleep(min(2**attempt, 20))
    raise RuntimeError(f"{url}: {last!r}")


def archive_identity(market: str, kind: str, symbol: str, month: str, cache: Path) -> tuple[Path, str]:
...
def ensure_archive(job: tuple[str, str, str, str], cache: Path) -> dict:
    market, kind, symbol, month = job
    path, url = archive_identity(market, kind, symbol, month, cache)
    checksum_path = path.with_name(path.name + ".CHECKSUM")
    if not checksum_path.exists():
        checksum_path.write_bytes(http_bytes(url + ".CHECKSUM"))
    ...
    if not path.exists() or sha256_file(path) != expected:
        ...
        temporary.write_bytes(http_bytes(url))
...
def run(cache: Path, output: Path, workers: int) -> dict:
    ...
    for symbol in SYMBOLS_CONTEXT:
        for month in MONTHS:
            jobs.append(("futures", "klines", symbol, month))
    records = []
    with ThreadPoolExecutor(max_workers=max(1, workers)) as pool:
        futures = [pool.submit(ensure_archive, job, cache) for job in jobs]
    ...
    for symbol in SYMBOLS_TRADED:
        ...
        for month in MONTHS:
            spot_path, _ = archive_identity("spot", "klines", symbol, month, cache)
            futures_path, _ = archive_identity("futures", "klines", symbol, month, cache)
            mark_path, _ = archive_identity("futures", "markPriceKlines", symbol, month, cache)
    ...
    for symbol in SYMBOLS_CONTEXT:
        parts = []
        for month in MONTHS:
            path, _ = archive_identity("futures", "klines", symbol, month, cache)
            parts.append(read_kline(path))
        summaries.append(write_segmented(pd.concat(parts, ignore_index=True), output, symbol, "fut"))
    result = {
        "candidate_id": "v25p_phase_memory_crossmarket_v1",
        "candidate_sha256": "923f011998e16836f30c8853b23d84a55103b68013785978b134ec4a414b5ff6",
