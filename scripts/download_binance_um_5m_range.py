...
def _archive_url(symbol: str, month: str) -> str:
    ...
    return f"{ARCHIVE_ROOT}/{symbol}/5m/{filename}"
...
def _fetch(url: str, *, retries: int) -> bytes:
    ...
    request = urllib.request.Request(
        url,
        headers={"User-Agent": "smc-trading-system-v09-5m/1.0"},
    )
    ...
    with zipfile.ZipFile(io.BytesIO(payload)) as archive:
        ...
        for member in members:
            ...
            if raw.shape[1] < len(KLINE_COLUMNS):
                raise ValueError(
                    f"{symbol}/{month}: unexpected kline columns={raw.shape[1]}"
                )
...
def _validate_range(
    ...
    *,
    ...
    outside = selected.index[(selected.index < start) | (selected.index >= end)]
    if len(outside):
        raise ValueError(f"{symbol}: selected timestamps outside requested range")
    if len(missing):
        preview = ", ".join(item.isoformat() for item in missing[:5])
        raise ValueError(
            f"{symbol}: missing {len(missing)} of {len(expected)} 5m bars; first={preview}"
        )
...
def main() -> int:
    ...
    if start.second or start.microsecond or start.nanosecond or start.minute % 5:
        raise ValueError("start must align to a 5-minute UTC boundary")
    if end.second or end.microsecond or end.nanosecond or end.minute % 5:
        raise ValueError("end must align to a 5-minute UTC boundary")
    ...
    manifest = {
        "schema_version": 1,
        "source": "Binance USD-M public monthly 5m kline archives",
        "archive_root": ARCHIVE_ROOT,
        "start": start.isoformat(),
        "end": end.isoformat(),
        "months": list(months),
        "symbols": list(symbols),
        "expected_rows_per_symbol": int(len(pd.date_range(start, end, freq="5min", inclusive="left"))),
        "sources": sources,
        "outputs": outputs,
    }
    (args.output_dir / "v09_5m_data_manifest.json").write_text(
