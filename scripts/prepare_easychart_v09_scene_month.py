...
from ictbt.microstructure import adapt_authority_to_flow_scene
...
def _registered_month(value: str) -> tuple[pd.Timestamp, pd.Timestamp]:
    ...
    if begin < utc(RESEARCH_START) or end > utc(HOLDOUT_END):
        raise ValueError(f"month {value} is outside the registered V0.9 interval")
    return begin, end
...
def _record(
    ...
    *,
    ...
    return record_from_adapted_scene(
...
def main() -> int:
    ...
    for family, authorities in (("v05", v05), ("v07", v07)):
        for authority in authorities:
            try:
                records.append(_record(authority, tick_size=tick_size))
            except ValueError as exc:
