...
단순히 OB 발생 횟수를 세는 프로그램이 아니라, 큰 시간봉의 구조와 유동성 위치에서 시작해 낮은 시간봉의 실행 타점으로 연결되는 과정을 코드로 구현한다. 거래 기회를 억지로 만들지 않으면서 의미 있는 빈도와 비용 차감 후 양의 기대값을 함께 추구하고, 같은 전략·위험·주문 상태기를 paper와 live에서 공유하는 것이 최종 목표다.
